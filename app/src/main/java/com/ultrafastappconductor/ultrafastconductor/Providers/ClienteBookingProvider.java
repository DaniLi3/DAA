...
public class ClienteBookingProvider {
    ...
    public ClienteBookingProvider() {
        mDatabase = FirebaseDatabase.getInstance().getReference().child("SolicitudesAceptadas");
    }

    public Task<Void> create(SolicitudesCanceladasmodel clientBooking) {
        return mDatabase.child(clientBooking.getIdviaje()).setValue(clientBooking);
    }
